...
class DeleteSaleHistoryTest{
    ...
    private lateinit var deleteSaleHistory: DeleteSaleHistory
    ...
    fun setup(){
        ...
        deleteSaleHistory = DeleteSaleHistory(fakeRepository)
    ...
    @Test
    fun deleteSaleHistory_s() = runBlocking{
        val string = "string"
        ...
        if(insertTest){
            try {
                ...
                assert(historyTest)
                // deleting sale history
                if(inventoryTest && historyTest){
                    val deletedHistory = histories.first{
                        it.createdTimeStamp == newInventory.createdTimeStamp
                    }
                    deleteSaleHistory(history = deletedHistory)
                    val newHistories = getHistory().first()
                    var deleteTest = true
                    for(i in newHistories){
                        if(i == deletedHistory){
                            deleteTest = false
                        }
                    }
